...
import { convertFolderTreeToObject, cleanEmptyEntries } from '../scripts/conversion-utils';
...
describe('Convertion of 3akai-ux properties files', () => {
  ...
  after(async () => {
    try {
      await util.promisify(fs.access)(repoPath, fs.constants.F_OK | fs.constants.W_OK);
  ...
  const setupContext = async locale => {
    convertedTranslations = await convertFolderTreeToObject(locale, repoPath, {}, true).then(cleanEmptyEntries);

    noKeysOfNestedPath = compose(keys, path(__, convertedTranslations), split('.'));
  ...
  it('British English json file is generated', async () => {
    ...
    checkNestedEntryBundles('packages.oae-jitsi-widgets.aboutmeeting-jitsi.bundles', 2);
    checkNestedEntryBundles('packages.oae-jitsi-widgets.meetings-jitsi-library.bundles', 5);
    checkNestedEntryBundles('packages.oae-jitsi-widgets.editmeeting-jitsi.bundles', 10);
    checkNestedEntryBundles('packages.oae-jitsi-widgets.createmeeting.bundles', 10);
    ...
    checkNestedEntryBundles('packages.oae-core.foldercontentvisibility.bundles', 16);
    ...
    // lets make sure the ${parameters} syntax is now just {parameters} with a random key
    const randomTranslation = convertedTranslations.shared.oae.bundles.ui.ACTIVITY_CONTENT_CREATE_FILE;
    assert.lengthOf(randomTranslation.split('$'), 1);
    assert.lengthOf(randomTranslation.split('{'), 3);
  });
